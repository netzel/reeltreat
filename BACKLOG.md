...
## Done
- Manual-shot upload from the Studio UI — upload an image on the Manifest screen; the bridge saves it into projects/<name>/manual/ and appends an image shot to the manifest via the YAML document API (comments/formatting preserved, result re-validated). Removed leftover dead UI state.

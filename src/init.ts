...
import { manifestPath, projectDir } from "./paths.js";
...
export function buildManifestYaml(input: ManifestYamlInput): string {
  ...
  out.push("");
  out.push("# Generated by `npm run init`. Edit freely — see projects/example/manifest.yaml for all options.");
  out.push("");
  ...
  if (brandLines.length > 0) {
    ...
    out.push("# brand: (none auto-detected — see projects/example/manifest.yaml to add a logo/colors)");
  }
  out.push("");
